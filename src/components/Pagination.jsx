...
const Pagination = ({query,currentPage,pagesNumber,nextPage,previousPage}) => {
  return (
    <div className='flex flex-row flex-wrap place-content-center'>
      ...
      {(currentPage !== 1) && <NavLink to={previousPage} className='w-6 h-fit font-bold p-1 m-2 bg-blue-900 text-center'>{"<"}</NavLink>}
      {[...Array(pagesNumber)].map((page,index) => {
        let range= [currentPage, currentPage+9]

        if((currentPage+9) <= (pagesNumber)){
          if((index+1 === range[0])){
            return(<span key={index} className='cursor-default w-6 h-fit p-1 m-2 bg-green-600 text-black text-center'>{index+1}</span>);
          }
      ...
      {(currentPage !== pagesNumber) && <NavLink to={nextPage} className='w-6 h-fit font-bold p-1 m-2 bg-blue-900 text-center'>{">"}</NavLink>}
    </div>
  );
}
